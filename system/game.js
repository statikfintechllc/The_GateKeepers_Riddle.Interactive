...
async function requestAICurated() {
    ...
    // Create GitHub issue URL with pre-filled template that triggers Copilot
    const issueTitle = encodeURIComponent('🤖 AI Riddle Request - Copilot Task');
    const issueBody = encodeURIComponent(`@copilot Please use the riddle-finder-agent to create a new high-quality riddle for this repository.

## Task Details
Create a new riddle PR using the riddle finder agent with the following specifications:
...
4. Create a PR with the new riddle
5. Auto-merge if all validation passes
...
*This request will be processed by GitHub Copilot working with the Riddle Finder Agent.*`);
    ...
    const issueLabels = 'ai-riddle-request,copilot';
    const repoUrl = 'https://github.com/statikfintechllc/The_GateKeepers_Riddle.Interactive';
    const issueUrl = `${repoUrl}/issues/new?title=${issueTitle}&body=${issueBody}&labels=${issueLabels}`;
    
    // Open GitHub issue creation page - user just needs to click Submit
    window.open(issueUrl, '_blank');
    
    // Show improved feedback message
    const feedback = document.querySelector('.feedback');
    if (feedback) {
        feedback.textContent = '🚀 Opening your AI riddle request - just click "Submit new issue" to start the process!';
